...
la liste des en-têtes de la réponse HTTP du serveur: 

    HTTP/1.1 200 OK
    Date: Fri, 22 Sep 2023 06:17:16 GMT
...
<!-- ###### -->
la liste des en-têtes qui ont changé depuis la version précédente ( --reponse): 

HTTP/1.1 200 OK

  --Content-Type: application/json (+ ajouter)

Date: Fri, 22 Sep 2023 06:28:48 GMT
Connection: keep-alive
Keep-Alive: timeout=5

  --Transfer-Encoding: chunked (- absent)

  --Content-Length: 20 (+ ajouter)

===> 'Content-Type' et 'Content-Length' ont etaient ajouter a la liste
     'Transfer-Encoding' a etait retiré
...
<!-- ###### -->
[Error: ENOENT: no such file or directory, open 'C:\Users\PC\Documents\Cours\UNC\L2\S4\DevWeb\devweb-tp5\index.html'] {
  errno: -4058,
  code: 'ENOENT',
  syscall: 'open',
  path: 'C:\\Users\\PC\\Documents\\Cours\\UNC\\L2\\S4\\DevWeb\\devweb-tp5\\index.html'
}
[Error: ENOENT: no such file or directory, open 'C:\Users\PC\Documents\Cours\UNC\L2\S4\DevWeb\devweb-tp5\index.html'] {
  errno: -4058,
  code: 'ENOENT',
...
<!-- ###### -->
un dossier node_modules a été ajouter dans le projet
et des fichiers tel que des package-lock.json 
<!-- ###### -->
...
<!-- ###### -->
http-prod ne s'actualise pas automatiquement lors de l'ajout de la commande `console.log` alors que http-dev le fait
<!-- ###### -->
...
- `http://localhost:8000/random.html` : 21
...
loglevel :
https://www.npmjs.com/package/loglevel        /   https://tidelift.com/funding/github/npm/loglevel
...
http://localhost:8000/random/5: Fonctionne renvoie la liste de random
http://localhost:8000/: Fonctionne renvoie le site

Pour le nomp run express-dev:
http://localhost:8000/index.html: Fonctionne renvoie le site
http://localhost:8000/random/5: Fonctionne renvoie la liste de random
http://localhost:8000/: Fonctionne pas renvoie "Cannot GET /random.html"
...
http://localhost:8000/:
    ...
    X-Powered-By: Express                          xxxxxxxxxxxxxxxxx
    Content-Security-Policy: default-src 'none'    xxxxxxxxxxxxxxxxx
    X-Content-Type-Options: nosniff

    Content-Type: text/html; charset=utf-8
    Content-Length: 143
    Date: Wed, 27 Sep 2023 22:33:14 GMT
    Connection: keep-alive
    Keep-Alive: timeout=5
...
http://localhost:8000/random.html:
    ...
    Content-Type: text/html; charset=utf-8
    Content-Length: 80
    ETag: W/"50-qx3O5QCF46yDzzmEmQL6bj7j7pA"
    Date: Wed, 27 Sep 2023 22:35:54 GMT
    Connection: keep-alive
    Keep-Alive: timeout=5
...
http://localhost:8000/index.html:
    ...
    ETag: W/"3ae-18abb70b425"                     xxxxxxxxxxxxxxxxx
    Content-Type: text/html; charset=UTF-8
    Content-Length: 942
    Date: Wed, 27 Sep 2023 22:37:06 GMT
    Connection: keep-alive
    Keep-Alive: timeout=5
...
L'événement listening sur le serveur HTTP Express est déclenché quand le serveur a terminé son démarrage et est prêt à recevoir des connexions réseau sur le port configuré.

Plus précisément, quand on appelle app.listen(port) pour démarrer le serveur
...
<!-- ###### -->
Par défaut, l'option fallthrough est activée pour le middleware express.static
<!-- ###### -->
...
Dans un rafraîchissement normal, on peut obtenir soit un "304 Not Modified" (utilisation du cache), soit un "200 OK" (mise à jour du cache).
Dans un rafraîchissement forcé, on obtiendra toujours un "200 OK" car le navigateur ignore le cache.
